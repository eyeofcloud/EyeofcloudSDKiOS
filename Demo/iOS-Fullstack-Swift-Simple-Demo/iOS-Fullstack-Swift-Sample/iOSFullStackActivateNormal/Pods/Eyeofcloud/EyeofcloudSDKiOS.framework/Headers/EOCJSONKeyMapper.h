...
typedef NSString *(^EOCJSONModelKeyMapBlock)(NSString *keyName);
 ...
 *
 * In your model overwrite the + (EOCJSONKeyMapper *)keyMapper method and provide a EOCJSONKeyMapper
 * instance to convert the key names for your model.
 *
 * If you need custom mapping it's as easy as:
 * <pre>
 * + (EOCJSONKeyMapper *)keyMapper {
 * &nbsp; return [[EOCJSONKeyMapper&nbsp;alloc]&nbsp;initWithDictionary:@{@"crazy_JSON_name":@"myCamelCaseName"}];
 * }
 * </pre>
 * In case you want to handle underscore_case, **use the predefined key mapper**, like so:
 * <pre>
 * + (EOCJSONKeyMapper *)keyMapper {
 * &nbsp; return [EOCJSONKeyMapper&nbsp;mapperFromUnderscoreCaseToCamelCase];
 * }
 * </pre>
 */
@interface EOCJSONKeyMapper : NSObject

// deprecated
@property (readonly, nonatomic) EOCJSONModelKeyMapBlock JSONToModelKeyBlock DEPRECATED_ATTRIBUTE;
...
- (instancetype)initWithJSONToModelBlock:(EOCJSONModelKeyMapBlock)toModel modelToJSONBlock:(EOCJSONModelKeyMapBlock)toJSON DEPRECATED_MSG_ATTRIBUTE("use initWithModelToJSONBlock:");
+ (instancetype)mapper:(EOCJSONKeyMapper *)baseKeyMapper withExceptions:(NSDictionary *)exceptions DEPRECATED_MSG_ATTRIBUTE("use baseMapper:withModelToJSONExceptions:");
...
@property (readonly, nonatomic) EOCJSONModelKeyMapBlock modelToJSONKeyBlock;

/** Combined converter method
 * @param value the source name
 * @return EOCJSONKeyMapper instance
 */
...
/**
 * Creates a EOCJSONKeyMapper instance, based on the block you provide this initializer.
 * The parameter takes in a EOCJSONModelKeyMapBlock block:
 * <pre>NSString *(^EOCJSONModelKeyMapBlock)(NSString *keyName)</pre>
 ...
 */
- (instancetype)initWithModelToJSONBlock:(EOCJSONModelKeyMapBlock)toJSON;

/**
 * Creates a EOCJSONKeyMapper instance, based on the mapping you provide.
 * Use your EOCJSONModel property names as keys, and the JSON key names as values.
...
/**
 * Creates a EOCJSONKeyMapper based on a built-in EOCJSONKeyMapper, with specific exceptions.
 * Use your EOCJSONModel property names as keys, and the JSON key names as values.
 */
+ (instancetype)baseMapper:(EOCJSONKeyMapper *)baseKeyMapper withModelToJSONExceptions:(NSDictionary *)toJSON;
