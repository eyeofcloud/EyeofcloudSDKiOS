...
class EOCORIViewController: UIViewController {
    ...
    @IBAction func click(_ sender: Any) {
        self.eocClient?.track("用户点击按钮", userId: appDelegate.getUserId()!,attributes: appDelegate.getUserAttributes()!)
    ...
    override func viewDidLoad() {
        ...
        let variation =  self.eocClient?.activate("iOS-param", userId: appDelegate.getUserId()!, attributes: appDelegate.getUserAttributes())
        if variation?.variationKey == "优化版本#1" {
            ori.text = "版本A优化界面"
        }else if(variation?.variationKey == "原始版本"){
            ori.text = "版本A界面"
        }else{
            ori.text = "版本A界面"
        }
