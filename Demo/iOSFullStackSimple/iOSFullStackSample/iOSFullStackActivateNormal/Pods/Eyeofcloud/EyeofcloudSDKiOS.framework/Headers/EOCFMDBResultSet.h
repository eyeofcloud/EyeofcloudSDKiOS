...
@class EOCFMDBStatement;

/** Represents the results of executing a query on an `<EOCFMDBDatabase>`.
...
@interface EOCFMDBResultSet : NSObject

@property (nonatomic, retain, nullable) EOCFMDBDatabase *parentDB;
...
/** `EOCFMDBStatement` used by result set. */

@property (atomic, retain, nullable) EOCFMDBStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<EOCFMDBStatement>`
 
 @param statement A `<EOCFMDBStatement>` to be performed
 
 @param aDB A `<EOCFMDBDatabase>` to be used
 
 @return A `EOCFMDBResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(EOCFMDBStatement *)statement usingParentDatabase:(EOCFMDBDatabase*)aDB;
