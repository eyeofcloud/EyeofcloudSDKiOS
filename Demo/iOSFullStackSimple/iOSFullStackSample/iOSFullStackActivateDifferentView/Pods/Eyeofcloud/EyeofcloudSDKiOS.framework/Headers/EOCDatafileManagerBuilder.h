...
@class EOCDatafileManagerBuilder;
@protocol EOCErrorHandler, EOCLogger;

/// This is a block that takes the biulder values
typedef void (^EOCDatafileManagerBuilderBlock)(EOCDatafileManagerBuilder * _Nullable builder);

@interface EOCDatafileManagerBuilder : NSObject

/** The time interval to regularly fetch the datafile.
 * The default time interval is 3600. This means that the datafile manager will regularly poll for a new datafile every 3600 seconds during the app session.
 */
...
@property (nonatomic, readwrite, strong, nonnull) id<EOCLogger> logger;
/// Datafile Download URLs
@property (nonatomic, strong, nullable) NSString *networkServiceCDNServerURL;
/// 
@property (nonatomic, assign, readwrite) BOOL datafileURLHasSetByClientSide;

/// Create an Eyeofcloud Datafile Manager Builder object.
+ (nullable instancetype)builderWithBlock:(nonnull EOCDatafileManagerBuilderBlock)block;
